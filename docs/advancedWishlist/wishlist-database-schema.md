# Database Schema - Advanced Wishlist System
...
```sql
-- Customer lookup
CREATE INDEX `idx.wishlist.customer_lookup` 
ON `wishlist` (`customer_id`, `type`, `is_default`);

-- Product popularity
CREATE INDEX `idx.wishlist_item.popularity` 
ON `wishlist_item` (`product_id`, `added_at`);

-- Share performance
CREATE INDEX `idx.share.performance` 
ON `wishlist_share` (`active`, `type`, `views` DESC);

-- Analytics queries
CREATE INDEX `idx.analytics.date_range` 
ON `wishlist_analytics` (`date`, `wishlist_id`);
```

### Composite Indexes

```sql
-- For wishlist overview page
CREATE INDEX `idx.wishlist.overview` 
ON `wishlist` (`customer_id`, `created_at` DESC)
INCLUDE (`name`, `item_count`, `type`);

-- For price monitoring
CREATE INDEX `idx.wishlist_item.price_monitor` 
ON `wishlist_item` (`price_alert_active`, `product_id`, `price_alert_threshold`)
WHERE `price_alert_active` = 1;

-- For share analytics
CREATE INDEX `idx.share_view.analytics` 
ON `wishlist_share_view` (`share_id`, `viewed_at`, `purchased`);
```
