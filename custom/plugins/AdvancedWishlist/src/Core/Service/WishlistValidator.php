...
class WishlistValidator
{
    ...
    public function validateCreateRequest(
        ...
        if (mb_strlen($request->getName()) > 255) {
            throw new WishlistException('Wishlist name cannot exceed 255 characters', ['field' => 'name', 'maxLength' => 255]);
        }

        // Validate type
        if (!in_array($request->getType(), ['private', 'public', 'shared'], true)) {
            throw new WishlistException('Invalid wishlist type. Must be one of: private, public, shared', ['field' => 'type', 'allowedValues' => ['private', 'public', 'shared']]);
        }
    ...
    public function validateUpdateRequest(
        ...
        if (empty($request->getWishlistId())) {
            throw new WishlistException('Wishlist ID is required', ['field' => 'wishlistId']);
        }
        ...
        if (null !== $type && !in_array($type, ['private', 'public', 'shared'], true)) {
            throw new WishlistException('Invalid wishlist type. Must be one of: private, public, shared', ['field' => 'type', 'allowedValues' => ['private', 'public', 'shared']]);
        }
    }
    ...
    public function validateOwnership(
        ...
        if ($wishlist->getCustomerId() !== $customerId) {
            throw new WishlistException('You do not have permission to access this wishlist', ['wishlistId' => $wishlist->getId()]);
        }
    }
    ...
    public function canViewWishlist(
        ...
        if ('public' === $wishlist->getType()) {
            return true;
        }

        // Check if the wishlist is shared with the customer
        if ('shared' === $wishlist->getType() && $wishlist->getShareInfo()) {
