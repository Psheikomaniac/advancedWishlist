...
class QueryOptimizer
{
    ...
    public function createIndex(string $tableName, array $columns, bool $isUnique = false): bool
    {
        try {
            ...
            $indexes = $this->getTableIndexes($tableName);
            foreach ($indexes as $index) {
                if (count(array_diff($index['columns'], $columns)) === 0 && count(array_diff($columns, $index['columns'])) === 0) {
    ...
    private function getCommonQueryPatternsForTable(string $tableName): array
    {
        ...
        } elseif ($tableName === 'wishlist_item') {
            $commonPatterns[] = [
                'columns' => ['wishlist_id'],
                'type' => 'index'
            ];
        ...
        } elseif ($tableName === 'wishlist_share') {
            $commonPatterns[] = [
                'columns' => ['wishlist_id'],
                'type' => 'index'
            ];
        ...
        } elseif ($tableName === 'guest_wishlist') {
            $commonPatterns[] = [
                'columns' => ['guest_id'],
                'type' => 'unique'
            ];
        ...
        } elseif ($tableName === 'wishlist_analytics') {
            $commonPatterns[] = [
                'columns' => ['wishlist_id'],
                'type' => 'index'
            ];
